...
const std::string SQL_CREATE_TABLE =    "CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, code INT NOT NULL UNIQUE, name VARCHAR(60) NOT NULL DEFAULT \'No name\', quantity DEC(7,3) NOT NULL DEFAULT 0, price DEC(6,2) NOT NULL DEFAULT 0, total DEC(8,2) NOT NULL DEFAULT 0)";
...
const std::string SQL_ADD_PRODUCT =     "INSERT INTO " + TABLE_NAME + "(code, name, quantity, price) VALUES(?, ?, ?, ?)";
...
const std::string SQL_UPDATE_PRODUCT =  "UPDATE " + TABLE_NAME + " SET code=?, name=?, quantity=?, price=? WHERE id=?";
